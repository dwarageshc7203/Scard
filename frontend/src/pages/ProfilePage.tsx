...
const ProfilePage: FC<ProfilePageProps> = ({ users, variant, initialUserId }) => {
  ...
  return (
    <div className="flex bg-bg/95 relative overflow-hidden" style={{ height: 'calc(100vh - 64px)' }}>
      
      {/* Sidebar with toggle width */}
      {variant === 'directory' && (
        <div 
          className={`transition-all duration-300 border-r border-border/40 overflow-hidden h-full ${
      ...
      <main className="flex-1 overflow-y-auto p-6 md:p-12 relative bg-[#141414]">
          ...
          <div className="space-y-8 pt-4">
            ...
            {selectedUser.pdfUrl && (
              <div className="space-y-2">
                <span className="text-[10px] text-muted uppercase tracking-widest font-bold block">PDF Resume</span>
                <a 
                  href={selectedUser.pdfUrl} 
            ...
            {/* Username Field */}
            <div className="space-y-1">
              <span className="text-[10px] text-muted uppercase tracking-widest font-bold block">Username</span>
              <span className="text-xs font-semibold text-text">{selectedUser.username}</span>
            </div>

            {/* Designation Field */}
            {selectedUser.designation && (
              <div className="space-y-1">
                <span className="text-[10px] text-muted uppercase tracking-widest font-bold block">Designation</span>
                <span className="text-xs font-semibold text-text">{selectedUser.designation}</span>
              </div>
            )}
      ...
      <div 
        ...
        style={{ left: variant === 'directory' && sidebarOpen ? '280px' : '24px' }}
      >
        {variant === 'directory' && (
          <button
