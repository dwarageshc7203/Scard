            ...
            <div className="flex gap-6 items-center">
              ...
              <div className="space-y-1">
                <div className="flex items-center gap-2.5 flex-wrap">
                  ...
                  {user.isOnline && (
                    <Badge variant="default" className="text-[9px] px-2 py-0 animate-pulse bg-green-500/10 text-green-400 border-green-500/25">
                      Online
                    </Badge>
                  )}
                </div>
                <p className="text-xs sm:text-sm text-muted font-medium">{user.title}</p>
                <div className="flex items-center gap-2 text-[10px] text-muted font-mono pt-1">
                  <Globe className="w-3.5 h-3.5" />
                  <span>{user.username}.scard.app</span>
                  <span>·</span>
                  <span className="font-semibold text-text/80">{user.totalContributions.toLocaleString()} contributions</span>
                </div>
              </div>
            </div>
