  ...
  return (
    ...
    >
      ...
      {isExpanded ? (
          ...
          {displayBadges.map((badge, i) => (
            ...
            >
              {badge.iconUrl ? (
                ...
                <Badge
                  variant={badge.platform}
                  className="text-xs uppercase tracking-wider px-3 py-1 z-10"
                >
        ...
        <div className="flex flex-wrap justify-center items-center gap-6 p-2 mx-auto">
          ...
          {hasMore && (
            ...
            >
              <span className="text-[10px] uppercase font-mono tracking-widest mt-1 mb-1">
      ...
      {showAll && !isExpanded && (
        ...
        >
          <div
            className="bg-surface p-6 rounded-2xl border border-border flex flex-col items-center gap-6 max-w-3xl max-h-[85vh] overflow-y-auto shadow-2xl relative"
          ...
          >
            ...
            <h3 className="text-gray-900 dark:text-white text-xl w-full text-center">
      ...
      {selectedBadge && (
        <div
          className="absolute inset-0 z-50 flex items-center justify-center bg-white/90 dark:bg-[#252525]/90 backdrop-blur-md rounded-[16px] animate-in fade-in zoom-in-95 duration-200"
          onClick={() => setSelectedBadge(null)}
        >
          <div
            className="bg-white dark:bg-[#2A2A2A] p-6 rounded-[20px] border border-gray-200 dark:border-white/10 flex flex-col items-center gap-4 max-w-[90%] shadow-2xl"
            onClick={(e) => e.stopPropagation()}
          >
            ...
            <div className="text-center space-y-1 mt-2">
              <h3 className="text-gray-900 dark:text-white text-lg tracking-tight">
                {selectedBadge.label}
              </h3>
              <p className="text-xs text-gray-500 dark:text-gray-400 font-mono uppercase tracking-widest">
                {selectedBadge.platform} Badge
              </p>
            </div>
            <button
              onClick={() => setSelectedBadge(null)}
              className="mt-4 px-4 py-1.5 rounded-full border border-gray-200 dark:border-white/10 text-xs text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-white hover:bg-gray-50 dark:hover:bg-white/5 transition-all"
            >
              Close
            </button>
          </div>
