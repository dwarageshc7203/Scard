...
const platformConfig: Record<string, { logo: string; color: string; label: string }> = {
  LEETCODE: {
...
const ProblemsSolved: React.FC<ProblemsSolvedProps> = ({ problems }) => {
  ...
  return (
    <div className="bg-white dark:bg-transparent border border-gray-200 dark:border-white/20 rounded-[20px] p-6 relative min-h-[300px] flex flex-col shadow-sm dark:shadow-none">
      <span className="text-[15px] text-gray-600 dark:text-gray-400 font-sans absolute top-5 left-6 z-10">Problems Solved</span>
      {platforms.length > 0 ? (
      ...
      {showAll && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in zoom-in-95 duration-200" onClick={() => setShowAll(false)}>
          <div className="bg-surface p-6 rounded-2xl border border-border flex flex-col gap-6 w-full max-w-3xl max-h-[85vh] shadow-2xl relative" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex flex-wrap justify-center gap-6 overflow-y-auto p-2 custom-scrollbar">
              {platforms.map(([platform, stats], idx) => {
                ...
                return (
                  <div key={idx} className="group relative bg-surface-2 rounded-xl p-4 border border-border/40 flex flex-col items-center justify-center text-center overflow-hidden transition-all duration-300 hover:border-[var(--hover-color)] hover:shadow-lg w-[140px] h-[140px]" style={{ '--hover-color': config.color } as React.CSSProperties}>
                    <div className="flex flex-col items-center transition-opacity duration-300 group-hover:opacity-0 group-hover:scale-95">
                      {config.logo ? (
