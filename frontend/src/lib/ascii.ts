export function processImageToAsciiText(
  ...
  let minGray = 255,
    maxGray = 0
  const grayPixels: number[][] = Array.from(
    { length: targetHeight },
    () => new Array(width),
  )
  ...
  for (let y = 0; y < targetHeight; y++) {
    for (let x = 0; x < width; x++) {
      ...
      const quantized =
        Math.round((oldGray * (levels - 1)) / 255) * (255 / (levels - 1))
...
export function generateAsciiFromBase64(
  base64: string,
  width: number = 90,
): Promise<string> {
  return new Promise((resolve, reject) => {
